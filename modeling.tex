...
The initial temperature and humidity field was horizontally homogeneous and determined by averaging all Climate Forecast System Reanalysis (CFSR) grid points within 100 km of Patricia's center of circulation at 1800 UTC 21 October 2015.
...
After an initial spin-up period of about 20 h, the modeled storm (Fig.~\ref{fig:vmax+pmin}, blue lines) began an RI period that lasted approximately 30 h.
After this RI, the storm continued to intensify more slowly until the maximum 10-m wind speed reached 89 m s\textsuperscript{-1} and the sea-level pressure reached its minimum of 846 hPa 81 h into the simulation.
...
\label{section:budgetdescription}
%---------------------------------------------------------------------------------------%
...
The budget changes are then averaged over 24-h periods and compared to the total model change in $N^2$ over that same time period, i.e.:
   ...
   \end{equation}
where $t_0$ is an initial time and $\delta t$ is 24 h.

Eqs. \ref{eq:budgetchange}--\ref{eq:residual} are plotted for three consecutive 24-h periods in Fig.~\ref{fig:mod+bud+res}.
...
In every 24-h period, the budget changes are nearly identical to the model changes, which is reflected in the near-zero residuals in the right column.
...
For this figure, each of the budget terms is computed using the method described in Section \ref{section:budgetdescription}, except with 1-h averaging intervals instead of 24-h intervals.
...
Over the subsequent 24 h, during the RI period, the $N^2$ within and above this layer decreased within the 25-km radius (Fig.~\ref{fig:n2-24hr-avgs}b).
...
These trends continued as the storm's intensity leveled off in the 48--72-h period (Fig.~\ref{fig:n2-24hr-avgs}c).
...
Additionally, when these waves reached r=0 km, a dipole of vertical velocity resulted, with ascent above and descent below.
For reasons that remain unclear, the regions of ascent were more persistent than the regions of descent, which resulted in the mean ascent observed near r=0 km above 17 km in Fig.~\ref{fig:adv-24-48}d.
...
During the initial spin-up period (0--24 h; Fig.~\ref{fig:qi+radten}a), convection was not deep enough to deposit large quantities of ice near the tropopause and create a persistent cirrus canopy.
...
During RI (24--48 h), the eyewall updraft strengthened and a radially-extensive cirrus canopy developed near the tropopause (Fig.~\ref{fig:qi+radten}c).
...
After the TC's RI period completed (48--72 h), strong radiative cooling remained near the tropopause at inner radii (Fig.~\ref{fig:qi+radten}f), sloping downward with the top of the cirrus canopy to below the tropopause at outer radii.
...
To compare our results to theirs, we ran a simulation identical to that described in Section \ref{modelsetup}, except using the NASA-Goddard radiation scheme and 625-m vertical grid spacing, to match those of \cite{Buetal2014}.
This simulation produced a maximum 24-h-average radiative cooling rate of 0.3 K h\textsuperscript{-1}, which agrees with that shown in \cite{Buetal2014}.
Another simulation using 625-m vertical grid spacing and RRTMG radiation produced 24-h-average cooling rates of up to 0.6 K h\textsuperscript{-1}.
...
Although the details of the intensification and the tropopause-layer $N^2$ evolution varied when these quantities were changed, the overall conclusions remain the same.
...
As an example, the maximum wind speed and minimum sea-level pressure for two of these simulations are plotted in Fig. \ref{fig:vmax+pmin-2runs-init}.
One simulation (Fig. \ref{fig:vmax+pmin-2runs-init}, red lines) is identical to that described in Section \ref{modelsetup}.
The second simulation (Fig. \ref{fig:vmax+pmin-2runs-init}, blue lines) also uses the same setup, except the initial sounding was determined by averaging every CFSR grid point within 1000 km of TC Patricia's storm center at 1800 UTC 21 October 2015 instead of averaging only within the 100-km radius.
The 100-km-average simulation (Fig. \ref{fig:vmax+pmin-2runs-init}, red lines) begins and completes its RI period sooner than the 1000-km simulation (Fig. \ref{fig:vmax+pmin-2runs-init}, blue lines), but the intensification rates and final intensity of the two simulations are quite similar.

The most notable difference between these two initial soundings is the dew point depression.
The 100-km-average sounding (Fig.~\ref{fig:cfsr-skewt}a) is more moist throughout the depth of the troposphere than the 1000-km average sounding (Fig.~\ref{fig:cfsr-skewt}b).
This more saturated environment, which should more readily support persistent organized convection, could account for the more rapid intensification of the 100-km-average TC.

%Although the less-rapid development in the 1000-km TC delays the strengthening of the lower-stratospheric stable layer, the $N^2$ evolution is quite similar once the RI begins.
Twenty-four-hour averages of $N^2$ from the 1000-km-average simulation are plotted in Fig. \ref{fig:n2-1000km}.
Although the lower-stratospheric stable layer developed more slowly than that shown in Fig.~\ref{fig:n2-24hr-avgs}, the overall evolution was quite similar once RI commenced, and the same budget terms dominated the tropopause-layer $N^2$ evolution (Fig.~\ref{fig:avgbudterms-run07}).

%Additionally, the 100-mb temperature was approximately 4\textdegree{}C colder in the 100-km-radius average sounding than in the 1000-km-radius average.
%This colder tropopause corresponded to smaller static stability in the upper troposphere, but larger static stability in the lower stratosphere.

 \subsection{Sensitivity to the vertical mixing length}
\label{lvsensitivity}
The rate of turbulent mixing in the Smagorinsky scheme used here is highly dependent on a prescribed length scale.
The vertical mixing length of the simulation described in Section \ref{modelsetup}, 100 m, was based on the sensitivity experiments of \cite{Bryan2012}.
%Prescribing a smaller mixing length produces smaller $\theta$ tendencies due to turbulence, but even with a mixing length on the low end of those tested by \cite{Bryan2012}, turbulence still plays an important role in the tropopause-layer $N^2$ evolution.
Prescribing a smaller vertical mixing length (50 m) delays rapid intensification (Fig. \ref{fig:vmax+pmin-2runs-lv}, blue lines), but the final intensity of this storm closely matches that in the simulation that uses a 100-m mixing length (Fig. \ref{fig:vmax+pmin-2runs-lv}, red lines).

The $N^2$ evolution of this simulation (Fig. \ref{fig:n2-50m}) is quite similar to that observed in the simulation which used a 100-m vertical mixing length (Fig. \ref{fig:n2-24hr-avgs}), although the lower-stratospheric $N^2$ maximum developed more slowly (compare Figs. \ref{fig:n2-24hr-avgs}b, \ref{fig:n2-50m}b).
Although the magnitude of the tropopause-layer $N^2$ forcing by vertical turbulence was smaller in this simulation than in the 100-m-mixing-length simulation (compare Figs. \ref{fig:avgbudterms}, \ref{fig:avgbudterms-run14}, blue lines), vertical turbulence still played an important role in destabilizing the layer immediately surrounding the tropopause and stabilizing the lower stratosphere (Fig. \ref{fig:turb-50m}).
...
The largest changes in $N^2$ occurred in a relatively shallow layer immediately surrounding the tropopause (Fig.~\ref{fig:fulltrop}, top).
This shallow layer also contained the largest diurnally-averaged radiative heating tendencies found anywhere in the domain (Fig.~\ref{fig:fulltrop}, bottom).
...
Values of vertical eddy diffusivity larger than any found outside of the boundary layer also resided in the upper troposphere (Fig.~\ref{fig:fulltrop}, middle).
...
The results herein suggest that this turbulence not only develops as a response to the presence of small static stability and large vertical wind shear, as discussed by \cite{Molinarietal2014} and \cite{DuranMolinari2016}, but also can actively increase the static stability in highly localized regions just above and below mixed layers.
...
In this chapter, all of the variables were averaged over a full diurnal cycle to eliminate the effects of diurnal variability and isolate the overall storm evolution.
...
\caption{Left panels: Twenty-four-h changes in squared Brunt-V{\"a}is{\"a}l{\"a} frequency ($N^2$; 10\textsuperscript{-4} s\textsuperscript{-2}) computed using Eq.~\ref{eq:modelchange} over (top row) 0--24 h, (middle row) 24--48 h, (bottom row) 48--72 h.
...
%FIGURE 3%
\begin{figure}[ht]
\centerline{\includegraphics[width=27pc]{figures/AVG_budterms.png}}
...
For each budget term, the absolute value of the $N^2$ tendency is averaged temporally over 1-h periods (using output every minute), and spatially in a region extending from 0 to 200-km radius and 14 to 21-km altitude.}
...
\caption{Twenty-four-hour averages of squared Brunt-V{\"a}is{\"a}l{\"a} frequency ($N^2$; 10\textsuperscript{-4} s\textsuperscript{-2}) over (a) 0--24 h, (b) 24--48 h, (c) 48--72 h.
Orange lines represent the cold-point tropopause height averaged over the same time periods.}
...
\caption{As in Fig.~\ref{fig:stab-00-24}, but for the 24--48-h period.}
...
\caption{As in Fig.~\ref{fig:stab-00-24}, but for the 48--72-h period.}
...
\caption{The contributions to the change in $N^2$ over the 24--48-h period (10\textsuperscript{-4} s\textsuperscript{-2} (24 h)\textsuperscript{-1}) by (a) horizontal advection and (b) vertical advection. (c) The radial velocity (m s\textsuperscript{-1}; filled contours), potential temperature (K; thick black contours), cold-point tropopause height (orange line), and level of maximum outflow (dashed cyan line) averaged over the 24--48-h period. (d) The vertical velocity (cm s\textsuperscript{-1}; filled contours), potential temperature (K; thick black contours), and cold-point tropopause height (orange line) averaged over the 24--48-h period.}
...
\end{figure*}
\begin{figure}
\caption{Ice mixing ratio (g kg\textsuperscript{-1}) and cold-point tropopause height (orange lines) averaged over (a) 0--24 h, (c) 24--48 h, and (e) 48--72 h. Radiative heating rate (K h\textsuperscript{-1}) and cold-point tropopause height (orange lines) averaged over (b) 0--24 h, (d) 24--48 h, and (f) 48--72 h.}
\label{fig:qi+radten}
\end{figure}
...
\caption{Vertical eddy diffusivity (m\textsuperscript{2} s\textsuperscript{-1}; filled contours), cold-point tropopause height (cyan lines), and radial velocity (m s\textsuperscript{-1}; thick black lines) averaged over (a) 0--24 h, (b) 24--48 h, and (c) 48--72 h.}
\label{fig:diff}
\end{figure*}

%FIGURE 12%
\begin{figure*}[ht]
\centerline{\includegraphics[width=39pc]{figures/vmax+pmin_run12+run07.png}}
\caption{(a) The maximum 2-m wind speed (m s\textsuperscript{-1}) and (b) minimum sea-level pressure (hPa) for the simulations described in (red) Section \ref{modelsetup} and (blue) Section \ref{thermosensitivity}.}
\label{fig:vmax+pmin-2runs-init}
\end{figure*}

%FIGURE 13%
\begin{figure*}[ht]
\centerline{\includegraphics[width=39pc]{figures/cfsr-skewt-montage.png}}
\caption{Skew $T$--log $p$ diagrams depicting the initial soundings used in the simulations described in (a) Section \ref{modelsetup} and (b) Section \ref{thermosensitivity}.}
\label{fig:cfsr-skewt}
\end{figure*}

%FIGURE 14%
\begin{figure*}[ht]
\centerline{\includegraphics[width=39pc]{figures/n2-1000km.png}}
\caption{Twenty-four-hour averages of squared Brunt-V{\"a}is{\"a}l{\"a} frequency ($N^2$; 10\textsuperscript{-4} s\textsuperscript{-2}) over (a) 0--24 h, (b) 24--48 h, (c) 48--72 h, and (d) 72--96 h for the simulation described in Section \ref{thermosensitivity}. Orange lines represent the cold-point tropopause height averaged over the same time periods.}
\label{fig:n2-1000km}
\end{figure*}

%FIGURE 15%
\begin{figure*}[ht]
\centerline{\includegraphics[width=27pc]{figures/AVG_budterms_run07.png}}
\caption{As in Fig. \ref{fig:avgbudterms}, but for the simulation described in Section \ref{thermosensitivity}.}
\label{fig:avgbudterms-run07}
\end{figure*}

%FIGURE 16%
\begin{figure*}[ht]
\centerline{\includegraphics[width=39pc]{figures/vmax+pmin_run12+run14.png}}
\caption{(a) The maximum 2-m wind speed (m s\textsuperscript{-1}) and (b) minimum sea-level pressure (hPa) for the simulations described in (red) Section \ref{modelsetup} and (blue) Section \ref{lvsensitivity}.}
\label{fig:vmax+pmin-2runs-lv}
\end{figure*}

%FIGURE 17%
\begin{figure*}[ht]
\centerline{\includegraphics[width=39pc]{figures/n2-50m.png}}
\caption{As in Fig. \ref{fig:n2-1000km}, but for the simulation described in Section \ref{lvsensitivity}.}
\label{fig:n2-50m}
\end{figure*}

%FIGURE 18%
\begin{figure*}[ht]
\centerline{\includegraphics[width=27pc]{figures/AVG_budterms-50m.png}}
\caption{As in Fig. \ref{fig:avgbudterms}, but for the simulation described in Section \ref{lvsensitivity}.}
\label{fig:avgbudterms-run14}
\end{figure*}
